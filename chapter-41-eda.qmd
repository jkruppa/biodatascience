...
ggplot() +
  ...
  geom_point(data = jump_blood_tbl, aes(x = 0, y = jumplength), size = 2) +
...
#| message: false
#| warning: false
jump_blood_tbl <- read_delim("data/jump_fleas.txt") |> 
  select(feeding, jumplength) |> 
  filter(feeding == "human blood") 
```
...
#| warning: false
#| echo: false
mean_blood_jump <- mean(jump_blood_tbl$jumplength)

jump_blood_plot_tbl <- jump_blood_tbl |>
  mutate(e = jumplength - mean_blood_jump,
         sign = as_factor(ifelse(sign(e) == -1, "\U2012", "+")))

sum_e_blood_jump <- sum(abs(jump_blood_plot_tbl$e))
```
...
mean_blood_jump <- mean(jump_blood_tbl$jumplength)
sd_blood_jump <- sd(jump_blood_tbl$jumplength)

p1_fig_dist_example <- ggplot() +
  theme_book() +
  coord_cartesian(xlim = c(-0.25, 0.25), ylim = c(0, 60)) +
  geom_point(data = jump_blood_tbl, aes(x = 0, y = jumplength), size = 2) +
  geom_text_repel(data = jump_blood_tbl, aes(x = 0, y = jumplength, label = 1:7)) +
  scale_x_continuous(breaks = 0, labels = "blood") +
  scale_y_continuous(breaks = c(0, 20, mean_blood_jump, 40, 60),
                     labels = c("0", "20", round(mean_blood_jump,1), "40", "60")) +
  labs(x = "", y = "Jump length in [cm]")

p3_fig_dist_example <- ggplot() +
  theme_book() +
  coord_cartesian(xlim = c(0, 1.4), ylim = c(0, 60)) +
  geom_rect(aes(xmin = 0.4, xmax = 1.1, ymin = 0, ymax = mean_blood_jump), fill = "#B12A9080",
            color = "black") +
  geom_segment(aes(x = 0.75, xend = 0.75, 
               y = mean_blood_jump, yend = mean_blood_jump+ sd_blood_jump), 
               color = "#E16462FF", size = 1) +
  geom_segment(aes(x = 0.6, xend = 0.9, 
               y = mean_blood_jump+ sd_blood_jump, yend = mean_blood_jump+ sd_blood_jump), 
               color = "#E16462FF", size = 1) +
  scale_x_continuous(breaks = 0.75, labels = "blood") +
  scale_y_continuous(breaks = c(0, 20, mean_blood_jump, 40, 60),
                     labels = c("0", "20", round(mean_blood_jump,1), "40", "60")) +
  labs(x = "Flea host", y = "")  
...
p2_fig_dist_example <- ggplot() +
  ...
  geom_segment(aes(x = 0.75, xend = 0.75, 
               y = mean_blood_jump, yend = mean_blood_jump+ sd_blood_jump), 
               color = "#E16462FF", size = 1) +
  annotate("text", x = 0.8, y = mean_blood_jump+ sd_blood_jump/2-0.5, hjust = "left", 
           label = expression(bold(s)), size = 5, color = "#E16462FF") +
  geom_path(data = tibble(x_raw = seq(mean_blood_jump-mean_blood_jump/1.5, 
                                      mean_blood_jump+mean_blood_jump/1.5, 0.1), 
                          y_raw = dnorm(x_raw, mean = mean_blood_jump, sd = sd_blood_jump)), 
            aes(y = x_raw, x = y_raw*20), 
            linewidth = 1, color = "black", angle = 90) +
  geom_segment(aes(x = -0.03, xend = 1.35, 
               y = mean_blood_jump, yend = mean_blood_jump), color = "#B12A90FF", size = 1) +
...
jump_blood_plot_tbl <- jump_blood_tbl |>
  mutate(e = round(jumplength - mean_blood_jump, 1),
         sign = as_factor(ifelse(sign(e) == -1, "\U2012", "+")),
         e_label = str_c(sign, abs(e)))

p1_fig_mean_example <- jump_blood_plot_tbl |> 
  ggplot(aes(x = 1:7, y = jumplength)) +
  theme_book() +
  geom_segment(aes(x = 1:7, xend = 1:7, y = jumplength, yend = mean_blood_jump, 
  ...
  geom_hline(yintercept = mean_blood_jump, color = "#B12A90FF", size = 1) +
  geom_point(size = 2) +
  scale_x_continuous(breaks = c(0:7), labels = c("All", 1:7)) +
  scale_y_continuous(breaks = c(25, 30, mean_blood_jump, 35, 40),
                     labels = c("25", "30", round(mean_blood_jump,1), "35", "40"),
                     limits = c(19, 44)) +
  labs(x = "blood flea ID", y = "") +
  ...
  geom_segment(data = filter(jump_blood_plot_tbl, sign == "\U2012"),
               aes(x = 4.8 + c(0, 0.3, 0.6), xend = 4.8 + c(0, 0.3, 0.6), 
                   y = (jumplength+20)/2, yend = (mean_blood_jump+20)/2, 
                   color = sign), show.legend = FALSE, size = 1) +
  geom_segment(data = filter(jump_blood_plot_tbl, sign == "+"),
               aes(x = 6 + c(0, 0.3, 0.6, 0.9), xend = 6 + c(0, 0.3, 0.6, 0.9), 
                   y = (jumplength+13)/2, yend = (mean_blood_jump+13)/2, 
                   color = sign), show.legend = FALSE, size = 1) +
  geom_text(data = filter(jump_blood_plot_tbl, sign == "\U2012"),
            aes(x = 4.8 + c(0, 0.3, 0.6), y = (jumplength+20)/2-0.5, 
                label = e_label, color = sign), fontface = 2, show.legend = FALSE, size = 3) +  
  geom_text(data = filter(jump_blood_plot_tbl, sign == "+"),
...
jump_blood_plot_tbl |> 
  ...
  annotate("tile", x = 28.25, y = 20, 
           width = sum_e_blood_jump/2.5, height = sum_e_blood_jump/2.5, fill = "#FCA63680",
           alpha = 0.2, color = "#FCA636FF", linewidth = 0.5) +
  ...
  scale_y_continuous(breaks = c(15, 20, 25, 30, mean_blood_jump, 35, 40),
                     labels = c("15", "20", "25", "30", round(mean_blood_jump,1), "35", "40"),
                     limits = c(NA, NA)) +
  labs(x = str_pad("blood flea ID", 122.5, "right"), y = "Jump length in [cm]") +
  scale_color_viridis(discrete = TRUE, option = "plasma", end = 0.9) +
...
ggplot() +
  ...
  annotate("tile", x = 14, y = -2.5, 
           width = sum_e_blood_jump/7, height = sum_e_blood_jump/7, fill = "#FCA63680",
           alpha = 0.2, color = "#FCA636FF", linewidth = 0.5) +
