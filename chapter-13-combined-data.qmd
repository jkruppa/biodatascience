...
```{r}
n_obs <- 41
jump_habitat_grid <- expand_grid(country = 1:9) |> 
  mutate(mean_country = rnorm(9, 0, 4)) |> 
  expand_grid(site = 1:3) |> 
  mutate(mean_site = rnorm(27, 0, 2)) |> 
  expand_grid(rep = 1:n_obs) |> 
  mutate(x = abs(rnorm(n = (9*3*n_obs), 2.5, 2.5)),
         y = 10 + 1.2 * x + mean_country + mean_site + rnorm(n = (9*3*n_obs), 0, 3)) |> 
...
lmer(y ~ x + (site|country), data = jump_habitat_grid)
