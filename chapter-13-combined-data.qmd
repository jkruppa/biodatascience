...
```{r}
n_obs <- 7
jump_habitat_grid <- expand_grid(country = 1:8) |> 
  mutate(mean_country = rnorm(8, 5, 1)) |> 
  expand_grid(site = 1:3) |> 
  mutate(mean_site = rnorm(24, 10, 1)) |> 
  expand_grid(rep = 1:n_obs) |> 
  mutate(x = abs(rnorm(n = (8*3*n_obs), 2.5, 2.5)),
         y = 10 + 1.2 * x + mean_country + mean_site + rnorm(n = (8*3*n_obs), 0, 1))
jump_habitat_grid |> 
  ggplot(aes(x, y, color = as_factor(site))) +
  geom_point() +
...
jump_habitat_raw_tbl <- jump_habitat_grid |> 
  rowwise() |> 
  mutate(jump_length_cm = lst(rnorm(7, mean, sd))) |> 
  unnest(cols = jump_length_cm)
```

```{r}
jump_habitat_tbl <- jump_habitat_raw_tbl |> 
  select(host, site, jump_length_cm) |> 
...
expand_grid(z_1 = 0:7, 
            z_2 = 0:2) |>
  mutate(z_1_eff = rnorm(24, 5, 0.001),
         z_2_eff = rnorm(24, 15, 0.001)) |> 
  expand_grid(rep = 1:n_obs) |> 
  mutate(x = abs(rnorm(n = (8*3*n_obs), 2.5, 2.5)),
         y = 10 * 1.2 * x + z_1_eff * z_1 + z_2_eff * z_2 + rnorm(n = (8*3*n_obs), 0, 0.001)) |> 
  ggplot(aes(x, y, color = as_factor(z_2))) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(~ z_1)
```
