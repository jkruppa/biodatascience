...
r2_tbl <- tibble(weight = abs(rnorm(5, 3, 4)),
                 jumplength = 10 + 1.2 * weight + rnorm(5, 0, 3))

r2_fit <- lm(jumplength ~ weight, data = r2_tbl)

mean_cat_jump <- mean(r2_tbl$jumplength)

r2_plot_tbl <- r2_tbl |> 
  mutate(sy = jumplength - mean(jumplength),
         e = residuals(r2_fit))

sum_sy <- (r2_plot_tbl$sy)^2 |> abs() |> sum() |> round(2)
sum_e <- (r2_plot_tbl$e)^2 |> abs() |> sum() |> round(2)
1-(sum_e/sum_sy)
```
...
r2_plot_tbl |> 
  ...
  annotate("segment", x = rep(-1, 7), xend = rep(11, 7),
           y = c(8, 10, 12, mean_cat_jump, 16, 18, 20), 
           yend = c(8, 10, 12, mean_cat_jump, 16, 18, 20),
           color = "gray95") +
  ...
  geom_segment(x = -1, xend = 11, 
               y = mean_cat_jump, yend = mean_cat_jump, color = "#B12A90FF", size = 1) +
  annotate("text", x = 11.5, y = mean_cat_jump, hjust = "left", label = expression(bold(bar(y))),
  ...
  geom_point() +
  geom_line(aes(y = predict(r2_fit)), color = "#0D0887FF") +
  ...
  geom_tile(aes(x = c(19, 21, 23, 25, 27), 
                ...
                width = e, height = e), fill = "#0D088780",
            alpha = 0.2, color = "#0D0887FF", linewidth = 0.5) +
#  geom_text(aes(x = c(19, 21, 23, 25, 27), 
#                y = c(17, 17, 17, 17, 17), label = round(e^2,1)),
#            fontface = 2) +
  ...
  annotate("text", x = 29.5, y = 8.5, label = expression(sum()*105.52), size = 5,
           hjust = "right") +
#  geom_text(aes(x = seq(20.5, 27, length.out =5),  
#                y = c(11.5, 11.5, 11.5, 11.5, 11.5), label = round(sy^2,1)),
#            fontface = 2) +
  annotate("text", x = 30.5, y = 15, hjust = "left", label = expression("= 0.86"),
           size = 6) +
  scale_x_continuous(breaks = c(0,5,10), limits = c(NA, 33.5),
                     expand = expand_scale(mult = 0)) +
  scale_y_continuous(breaks = c(8, 10, 12, mean_cat_jump, 16, 18, 20),
                     labels = c("8", "10", "12", round(mean_cat_jump, 1), "16", "18", "20"),
                     limits = c(NA, NA)) +
