...
cor_high_tbl |> 
  ...
  geom_segment(x = mean(cor_high_tbl$weight), 
               ...
               y = 7, yend = 25, color = "#B12A90FF", linetype = 11) +
  ...
  scale_color_viridis(discrete = TRUE, option = "plasma", end = 0.9) +
  ## upper right
  annotate("text", x = 13.5, y = 20, hjust = "right", label = expression(SS~"="),
            size = 5) + 
  ...
  geom_shape(data = tibble(x = c(22.25, 30.25, 30.25, 22.25), 
                           y = c(19.75, 19.75, 16, 16)),
             aes(x, y), radius = unit(0.2, 'cm'), fill = "gray95") +
#  annotate("text", x = 30.75, y = 20, hjust = "left", label = expression("="~120.33),
#            size = 5) + 
  ...
  geom_spit_tile(x = c(16.5), y = c(21.75), 
  ...
  annotate("text", x = 30, y = 23.25, label = expression(sum()*33.21), size = 4.5,
           hjust = "right") +
  geom_tile(aes(x = c(17.75, 19.5, 21, 21, 15.5), 
                y = c(18, 18.5, 17.75, 18.825, 18), 
                width = sjump/2, height = sjump/2), fill = "#0D088780",
            alpha = 0.2, color = "#0D0887FF", linewidth = 0.5) + 
  annotate("text", x = 21.75, y = 16.75, label = expression(sum()*47.94), size = 4.5,
           hjust = "right") +
  geom_tile(aes(x = c(23.5, 25.25, 26.75, 27.625, 29), 
                y = c(18, 18, 18, 18, 18.5), 
                width = sweight/2, height = sweight/2), fill = "#B12A9080",
            alpha = 0.2, color = "#B12A90FF", linewidth = 0.5) +
  annotate("text", x = 30, y = 16.75, label = expression(sum()*33.96), size = 4.5,
           hjust = "right") +
  ## lower right left
  annotate("text", x = 13.5, y = 12, hjust = "right", label = expression(r~"="),
            size = 5) + 
  annotate("segment", x = 13.75, xend = 24.75, y = 12, yend = 12) +
  geom_spit_tile(x = 19.25, y = 12.25+3/2, a = 3, b = 3, weight = 1,
                 color = c("#0D0887FF", "#B12A90FF"), fill = c("#0D088780", "#B12A9080")) +
  ...
  annotate("text", x = 16.5, y = 11.75-3+3/2, label = "47.94", fontface = 2) +
  annotate("tile", x = 22, y = 11.75-3+3/2, 
           width = 3, height = 3, fill = "#B12A9080",
           alpha = 0.1, color = "#B12A90FF", linewidth = 0.5, linetype = "33") +
  annotate("text", x = 22, y = 11.75-3+3/2, label = "33.96", fontface = 2) +
  annotate("point", x = 19.25, y = 10.25, size = 2) +
  ...
  annotate("text", x = 30.75, y = 12, hjust = "left", label = expression("="~0.86),
            size = 5) + 
  geom_spit_tile(x = 28.125, y = 12.25+3/2, a = 3, b = 3, weight = 1,
                 color = c("#0D0887FF", "#B12A90FF"), fill = c("#0D088780", "#B12A9080")) +
...
#ggplot() +
  #geom_spit_tile(x = 0, y = 0, a = 3, b = 2, weight = 1,
       #          color = c("red", "blue"), fill = c("gray50", "gray96"))# +
  #geom_shape(data = tibble(x = c(1, 3, 1), 
   #                        y = c(1, 4, 4)),
   #            aes(x, y))
