...
#| fig-height: 5
#| fig-width: 15
#| fig-cap: "Das kausale und prädiktive Modell im statistischen Modellieren für den simplen Fall. In allen Fällen ist der Outcome $y$ normalverteilt. **(A)** *Explanatory model* -- Wie verändert sich der Outcome $y$, wenn sich die kontinuierliche Influencer $x$ ändert? Wie ist der numerische Zusammenhang zwischen $y$ und $x$? Eine Frage nach der Steigung der Graden. **(B)** *Explanatory model* -- Wie verändert sich der Outcome $y$, wenn sich die kategoriale Influencer als Faktor $f_A$ ändert? Wie ist der numerische Zusammenhang zwischen $y$ und $f_A$? Eine Frage nach der Steigung der Graden. **(C)** *Prädikitives Modell* -- Wenn $y$ und $x$ gemessen wurden, wie sehen dann die Werte von $y$ für neue $x$-Werte aus? Können wir mit der Influencer $x$ die neuen Outcomee in $y$ vorhersagen? *[Zum Vergrößern anklicken]*"

set.seed(20240329)
cbbPalette <- c("#000000", "#FCA636FF", "#0D0887FF", "#009E73", 
                "#F0E442", "#0072B2", "#D55E00", "#B12A90FF")
