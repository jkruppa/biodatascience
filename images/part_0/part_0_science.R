...
people_tbl <- read_excel("data/people_of_intrest.xlsx") |> 
  filter(short %in% c("Kepler", "Kopernikus", "Newton", "Halley",
                      "Herschel", "Einstein", "Brahe"))

birth_tbl <- people_tbl |>
  arrange(birth) |> 
  mutate(short = fct_rev(as_factor(short)),
         long = fct_rev(as_factor(long))) |> 
  pivot_longer(cols = birth:death)

p_persons_sunsystem <- birth_tbl |> 
  ...
  ## first
  geom_point(data = people_tbl, aes(short, date_1, group = short), color = "#0D0887FF") +
  geom_text(data = people_tbl, aes(short, date_1, group = short, label = finding_1), 
            color = "#0D0887FF", position = position_nudge(x = 0.35), size = 3.5) +
  ## second
  geom_point(data = people_tbl, aes(short, date_2, group = short), color = "#B12A90FF") +
  geom_text(data = people_tbl, aes(short, date_2, group = short, label = finding_2), 
...
p_persons_statistics <- birth_tbl |> 
  ggplot(aes(x = short, y = value, group = short)) +
  theme_book() +
  geom_path(color = "gray25") +
  geom_point(size = 2, shape = "|") +
