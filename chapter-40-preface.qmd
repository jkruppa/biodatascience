      ...
      if(zero){
        list(
          ...
          annotate("text", x = x -0.25, y = y + seq(0, 3.5, 0.5), label = seq(0, 70, 10),
                   hjust = "right"),
          annotate("text", x = x - 0.9, y = y + 2, label = c("Mean count of hairleg"), fontface = 2, 
                   size = 4, angle = 90)
        )
      } else {
        list(
          ...
          annotate("text", x = x - 0.9, y = y +2, label = c("Mean jump length"), fontface = 2, size = 4, 
                   angle = 90)
        )        
      }
    },
    if(yticks == "3lvl") {
      list(
        ...
        annotate("text", x = x -0.25, y = y + (1:7)/2, label = c("1", "2", "3", "4", "5", "6", "7"), 
                 hjust = "right") ,
        annotate("text", x = x - 0.8, y = y +2, label = c("Average grade"), fontface = 2, size = 4, 
                 angle = 90)
      )
    },
    if(yticks == "2lvl") {
      list(
        ...
        annotate("text", x = x - 0.8, y = y +2, label = c("Mean infection rate"), fontface = 2, size = 4,
                 angle = 90)
      )
    },
    if(xticks == "2lvl") {
      list(
        ...
        annotate("text", x = x + 2, y = y -1, label = c("Host"), fontface = 2, size = 4)
...
ggplot() +
  theme_void() +
  ##theme_minimal() +
  coord_cartesian(xlim = c(2.25, 13.5), ylim = c(0.25, 13.75)) +
...
```{r}
sqrt((0.5764023	+ 0.7797969)/2)	
```


```{r}
fit <- lm(value ~ key, foo)
...
```{r}
library(nlme)
gls(value ~ key, weights = varIdent(form =  ~ 1 | key), foo) |> 
  summary()
```
