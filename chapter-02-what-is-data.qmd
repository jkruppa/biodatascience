...
geom_fire_alarm <- function(x, y, alarm = FALSE, fire = FALSE) {
  list(
    annotate("segment", x = x, xend = x, y = y, yend = y+0.5),
    annotate("segment", x = x+8, xend = x+8, y = y, yend = y+0.5),
    annotate("segment", x = x+9, xend = x+9, y = y+1, yend = y+1.5),
    # bottom
    ...
    geom_shape(data = tibble(x = c(x+8, x+9, x+9, x+8), 
               ...
               aes(x, y), fill = "gray50"),
    ## back
    annotate("rect", xmin = x +1, xmax = x + 8, ymin = y+1.5, ymax = y +8.5, fill = "gray80"),
    ## line
    annotate("segment", x =  x +0.5, xend = x+8, y = y + 8, yend = y +8)
  )
}
...
ggplot() +
  ...
  geom_fire_alarm(x = 0.5, y = 0.5, alarm = "TRUE")
```
